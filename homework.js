...
1. fetch 是瀏覽器原生的 API，而 axios 是一個第三方套件，需要額外安裝。
2. fetch 需要手動設定 Content-Type、headers等，而 axios 會自動設定。
3. fetch 只有在網路錯誤才會 throw error，而 axios 會包含狀態碼 400~599 的錯誤。
...
const OrderService = {
  ...
  formatOrders(orders) {
    ...
    return orders.map((order) => {
      return { ...order, formattedDate: formatOrderDate(order.createdAt) };
    });
  },
